//! JPEG encoding.
//!
//! Vendored from `jpeg-encoder` 0.7.0 (Volker Ströbel,
//! (MIT OR Apache-2.0) AND IJG). See `NOTICE.md` for attribution, the IJG
//! terms covering `fdct.rs`/`avx2/fdct.rs`, and the list of local changes.
...
#[cfg(test)]
mod tests {
    use crate::decode::{Decoder, ImageInfo, PixelFormat};
    use crate::encode::image_buffer::rgb_to_ycbcr;
    use crate::encode::{ColorType, Encoder, QuantizationTableType, SamplingFactor};
